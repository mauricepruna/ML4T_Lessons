...
class RTLearner(object):
    ...
    def addEvidence(self, X, Y):
        def built_tree(dataX, dataY):
            leaf = -1000
            if dataX.shape[
                0] <= self.leaf_size:  # checks if the data points in a branch has equal or less than the leaf size
                self.tree = np.asarray(
                    [[leaf, np.mean(dataY), np.nan, np.nan]])  # average the Y's  of data points in a leaf
                return self.tree
            elif len(set(dataY)) == 1:
                self.tree = np.asarray([[leaf, np.mean(dataY), np.nan,
                                         np.nan]])  # checks if all the Y values are the same, then just return Y
                return self.tree
            else:
                randomList = rd.sample(xrange(0, dataX.shape[1]),
                                       dataX.shape[1])  # randomly order the features in a list
                i = randomList[0]  # select the first feature in the list to split on
                z = 1
                while len(set(dataX[:, i])) == 1 and z <= dataX.shape[
                    1]:  # check if the feature has any information, if all data points in that feature is the same pick another feature
                    i = randomList[z]
                    z = z + 1
                if len(set(dataX[:,
